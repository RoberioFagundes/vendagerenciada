...
<div class="card mt-4 mb-6 border-light shadow">
    ...
    <form action="{{ route('update_fiado', ['id',$clientefiado->id])}}" method="post">
        @csrf
        @method('PUT')

        <input type="hidden" name="id" value="{{isset($clientefiado) ? $clientefiado->id : 0}}">
        <label for="">Nome:</label>
        <select class="form-control" name="cliente_id">
            <option value="{{ $clientefiado->cliente_id}}">{{ $clientefiado->nomeCliente }}</option> 
        </select>
        <br>
        <label for="">Valor total da Nota</label>
        <input  class="form-control" type="text" name="valorTotal" id="valorTotal"
         value="{{number_format($clientefiado->valor_total, 2, '.', '.') }}">
       
         <label for="">Data da Nota</label><br>
        <input class="form-control" type="text" name="dataNota" 
        value="{{ \Carbon\Carbon::parse($clientefiado->vencimento)->tz('America/Sao_Paulo')->format('d/m/Y') }}">
        <br>
