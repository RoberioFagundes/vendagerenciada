...
class TestevendaController extends Controller
{
...
"); //consulta para mostrar usuarios horario de vendas
        ...
        $valortotalCaixaTodosPeriodoClienteAnoTarde = DB::select("select  format(SUM(item_vendas.valor * item_vendas.quantidade),2)
         as valortotal
            from vendas,
             item_vendas,
              users,
        ...
        $valortotalCaixaTodosPeriodoClienteAnoNoite = DB::select("select  format(SUM(item_vendas.valor * item_vendas.quantidade),2) as valortotal
            from vendas,
             item_vendas,
              users,
                 produto_famarcia,
            ...
            item_vendas.venda_id=vendas.id and
            produto_famarcia.id=item_vendas.produto_id and
             fatura_vendas.venda_id=vendas.id and users.id=1
             and 
             YEAR(item_vendas.created_at)=YEAR(CURRENT_DATE()) 
             and TIME(item_vendas.created_at) > TIME('17:00:00') AND
              TIME(item_vendas.created_at) <= TIME('24:00:00')"); //consulta para mostrar usuarios horario de vendas
        ...
        $valortotalCaixaTodosPeriodoClienteAno =
         DB::select("select  format(SUM(item_vendas.valor * item_vendas.quantidade),2) as valortotal
            ...
            item_vendas.venda_id=vendas.id and
            produto_famarcia.id=item_vendas.produto_id and
             fatura_vendas.venda_id=vendas.id and users.id='$user_id'
             and 
             YEAR(item_vendas.created_at)=YEAR(CURRENT_DATE()) 
             "); //consulta para mostrar usuarios horario de vendas
                // dd($valortotalCaixaTodosPeriodoClienteAno);
...
SUM(vendas.valor) as valortotal
       ...
       produto_famarcia_unidades.id=itemvendas_unidade.produtofarmaciaUnidade_id and
        ...
        $somartotalCaixa=str_replace(",","",$valortotalCaixaDiaSemClienteManhaClienteDias);
        
        $somarTotalCaixa = floatval($somartotalCaixa);
       


        foreach ($valortotalVendaUnidadeSemClientePerimetroManhaDia as $ValortotalVendaUnidadeSemClientePerimetroManhaDia) {
            $ValortotalVendaUnidadeSemClientePerimetroManhaDias = $ValortotalVendaUnidadeSemClientePerimetroManhaDia->valortotal;
        }

        $somarTotalUnidade =str_replace(",","",$ValortotalVendaUnidadeSemClientePerimetroManhaDias);
        $somartotalUnidade= floatval($somarTotalUnidade);
        
        // dd($somartotalUnidade);
        ...
        $DayCaixaManha = $valortotalCaixaDiaSemClienteManhaClienteDias;
        // dd($valortotalVendaCartelaSemClientePerimetroManhaDay);
        
        $DayCartelaManha = floatval($ValortotalVendaCartelaSemClientePerimetroManhaDias);
        ...
        $somarCaixaUnidadeCartelaDay = $somarTotalCaixa + $somartotalUnidade;
        ...
        // dd($valortotalCaixaTodosPeriodoClienteAnoNoite);
        ...
        $TodoPeriodoAnoYearCaixaComUnidadeComCartela = $somarCaixaComUnidadeComCartelaYEARManha + $somarCaixaComUnidadeComCartelaYearTarde + $somarCaixaComUnidadeComCartelaYearNoite;
        ...
        return view('Teste/Venda/List', compact(
            'titulo',
            'vendasClienteCaixa',
    ...
    { //função correta
        ...
        try {
            ...
            // }
            $titulo = 'Nova venda';
            return view('Teste/Vendas/form', compact('titulo', 'clientes', 'produtos', 'produtosUnidade', 'produtoCartela'));
        } catch (\Exception $e) {
    ...
    public function show($id)
    {
        try {
            ...
            return view('Teste/vendas/show', compact('titulo', 'vendaCartela', 'vendacartela1'));
        } catch (\Exception $e) {
            session()->flash('erro', $e->getMessage());
            return redirect()->back();
        }
    ...
    public function gerarPdf(Request $request)
    {
        ...
        $vendasClienteCartela = User::Join('cidades', 'cidades.user_id', '=', 'users.id')
            ->Join('clientes', 'clientes.cidade_id', '=', 'cidades.id')
            ->Join('vendas', 'vendas.cliente_id', '=', 'clientes.id')
            ->Join('itemvendas_cartela', 'itemvendas_cartela.venda_id', '=', 'vendas.id')
            ->join('produto_famarcia_cartela', 'produto_famarcia_cartela.id', 'itemvendas_cartela.produtofarmaciaCartela_id')
            ...
            $totalvenda = User::join("vendas","vendas.user_id","=", "users.id")
        ...
        $pdf = PDF::loadView('vendas.gerar-pdf', [
            ...
            'titulo'
            => $titulo,
            'VendaCaixaPerimetroManhaSemCliente' => $VendaCaixaPerimetroManhaSemCliente,
            'VendaUnidadeClientePerimetroManha' => $VendaUnidadeClientePerimetroManha,
            'vendasClienteCartela' => $vendasClienteCartela,
    ...
    public function showpdf_cartela($id)
    {
        ...
        $pdf = PDF::loadView('vendas.gerar-cliente-cartela-pdf', [
            'VendaCartelaClientePerimetroManha' => $VendaCartelaClientePerimetroManha,
            'titulo' => $VendaCartelaClientePerimetroManha
        ])->setPaper('a4', 'portrait')->setOptions(['defaultFont' => 'sans-serif']);
