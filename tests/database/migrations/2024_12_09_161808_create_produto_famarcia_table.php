...
class CreateProdutoFamarciaTable extends Migration
{
    /**
     * Run the migrations.
     *
    ...
    public function up()
    {
        Schema::create('produto_famarcia', function (Blueprint $table) {
            $table->id();
            ...
            $table->foreign('USER_ID', 'produto_famarcia_ibfk_1')->references('id')->on('users')->onDelete('cascade')->onUpdate('cascade');
            $table->foreign('categoria_id', 'produto_famarcia_ibfk_2')->references('id')->on('categorias')->onDelete('cascade')->onUpdate('cascade');
        });
    }
    ...
    {
        Schema::dropIfExists('produto_famarcia');
    }
}
