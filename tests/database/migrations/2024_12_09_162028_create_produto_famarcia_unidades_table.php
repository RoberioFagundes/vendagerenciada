...
class CreateProdutoFamarciaUnidadesTable extends Migration
{
    /**
     * Run the migrations.
     *
    ...
    public function up()
    {
        Schema::create('produto_famarcia_unidades', function (Blueprint $table) {
            $table->id();
            $table->string('COD_BARRA', 30);
            $table->unsignedInteger('categoria_id')->nullable();
            $table->string('substancia', 300)->nullable();
            $table->string('indicacao', 300)->nullable();
            $table->string('cnpj', 25)->nullable();
            $table->string('laboratorio', 300)->nullable();
            $table->string('codigoGgrem', 50)->nullable();
            $table->bigInteger('registro_ms')->comment("são os registros do remedios");
            ...
            $table->decimal('QTD_PRODUTO', 10, 2)->nullable();
            $table->decimal('QTD_PRODUTO_UNIDADE', 10, 2)->nullable();
            ...
            $table->foreign('USER_ID', 'produto_famarcia_unidades_ibfk_1')->references('id')->on('users')->onDelete('cascade')->onUpdate('cascade');
            $table->foreign('categoria_id', 'produto_famarcia_unidades_ibfk_2')->references('id')->on('categorias')->onDelete('cascade')->onUpdate('cascade');
        });
    }
    ...
    {
        Schema::dropIfExists('produto_famarcia_unidades');
    }
}
